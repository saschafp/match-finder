...
RAW_HTML_ROOT = Path(__file__).resolve().parents[1] / "data" / "raw" / "matchcenter"
...
def main() -> None:
    with mc.MatchcenterClient() as client:
        for definition in definitions:
            print(f"Fetching landing page for {definition.name}...")
            ...
            write_html(
                RAW_HTML_ROOT / "landing" / f"{definition.key}.html",
                landing_html,
            )
            ...
            for schedule in schedules:
                print(f"Fetching schedule {schedule.key}...")
                ...
                write_html(
                    RAW_HTML_ROOT / "schedules" / f"{schedule.key}.html",
                    schedule_html,
                )
